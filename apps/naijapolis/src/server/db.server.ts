...
const SEED_TRANSACTIONS: Transaction[] = [
  { id: 't-1', person_id: 'p-1', person_name: 'Emeka Okonkwo', amount: 50000, channel: 'paystack', status: 'success', reference: 'PAY-001-2026', fundraiser_id: 'f-1', created_at: '2026-04-30T08:30:00Z' },
  { id: 't-2', person_id: 'p-5', person_name: 'Tunde Adeyemi', amount: 100000, channel: 'bank_transfer', status: 'success', reference: 'BT-002-2026', fundraiser_id: 'f-1', created_at: '2026-04-30T12:00:00Z' },
  { id: 't-3', person_id: 'p-4', person_name: 'Fatima Mohammed', amount: 75000, channel: 'paystack', status: 'success', reference: 'PAY-003-2026', fundraiser_id: 'f-2', created_at: '2026-05-02T07:00:00Z' },
...
const SEED_FUNDRAISERS: Fundraiser[] = [
...
const SEED_GOALS: Goal[] = [
  { id: 'g-1', name: 'Q2 Fundraising Target', description: 'Raise ₦5M for Q2 campaign operations', type: 'donation', target: 5000000, current_value: 255000, created_at: '2026-04-01T00:00:00Z' },
...
const SEED_CANVASS: CanvassRecord[] = [
...
const SEED_ADVOCACY: AdvocacyContact[] = [
...
export async function getActivities(): Promise<Activity[]> {
  await ensureSeeded()
  const all = await getAll<Activity>('activities')
...
export async function getFundraisers(): Promise<Fundraiser[]> {
  await ensureSeeded()
  return getAll<Fundraiser>('fundraisers')
}
...
export async function addEventRsvp(rsvp: EventRsvp): Promise<void> {
  const all = await getAll<EventRsvp>('event_rsvps')
  await setAll('event_rsvps', [...all, rsvp])
  // bump rsvp_count on event
  const events = await getEvents()
  await setAll('events', events.map(e =>
    e.id === rsvp.event_id ? { ...e, rsvp_count: e.rsvp_count + 1 } : e
  ))
}
...
export async function updateGoalValue(id: string, value: number): Promise<void> {
  const all = await getGoals()
  await setAll('goals', all.map(g => g.id === id ? { ...g, current_value: value } : g))
}

// Canvass
export async function getCanvassRecords(): Promise<CanvassRecord[]> {
  await ensureSeeded()
  return getAll<CanvassRecord>('canvass')
}

export async function addCanvassRecord(record: CanvassRecord): Promise<void> {
  const all = await getCanvassRecords()
  await setAll('canvass', [record, ...all])
}

// Advocacy
export async function getAdvocacyContacts(): Promise<AdvocacyContact[]> {
  await ensureSeeded()
  return getAll<AdvocacyContact>('advocacy')
}

export async function addAdvocacyContact(contact: AdvocacyContact): Promise<void> {
...
export async function getSettings(): Promise<AppSettings> {
  ...
  try {
    ...
    return (data as AppSettings) ?? SEED_SETTINGS
  } catch {
